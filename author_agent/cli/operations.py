...
def workflow_create(args: Any, *, store_factory: Callable[[], Any], output: Callable[[Any], None]) -> None:
    ...
    output(
        service.create(
            ...
            state=WorkflowState(args.state),
            text=args.text,
        )
    )
...
def workflow_transition(args: Any, *, store_factory: Callable[[], Any], output: Callable[[Any], None]) -> None:
    ...
    output(
        json.dumps(
            service.transition(args.id, WorkflowState(args.state), reason=args.reason), ensure_ascii=False, indent=2
        )
    )
...
def evaluate(args: Any, *, fixture_dir: Path, output: Callable[[Any], None]) -> None:
    ...
    output(
        "Factual precision: "
        f"{report['factual_precision']:.3f}; "
        "unsupported claim rate: "
        f"{report['unsupported_claim_rate']:.3f}"
    )

    output(
        "Duplicate rate: "
        f"{report['duplicate_rate']:.3f}; "
        "platform differentiation: "
        f"{report['platform_differentiation']:.3f}"
    )
...
def analyze_visual(
    ...
    *,
    ...
    evidence = analyzer.analyze_pdf(
        Path(args.book), enabled=enabled, max_pages=args.max_pages or int(cfg.get("max_pages", 0))
    )
...
def rag_sync_approved(
    *,
    root: Path,
    load_json: Callable[..., Any],
    approve_file: Callable[[Path, bool], dict],
    output: Callable[[Any], None],
) -> None:
    results = []
...
def dashboard(
    ...
    *,
    ...
    load_releases: Callable[[], list[dict]],
    build_dashboard: Callable[..., Any],
    validate_date: Callable[..., str],
    output: Callable[[Any], None],
