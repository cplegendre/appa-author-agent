...
class FakePublisher:
    ...
    def get_post_status(self, external_id, platform):
        return "published"

    def fetch_metrics(self, external_id, platform):
        return {"reach": 1.0}
...
def test_meta_permanent_error_normalized():
    client = httpx.Client(
        transport=httpx.MockTransport(lambda req: httpx.Response(400, json={"error": {"message": "bad token details"}}))
    )
